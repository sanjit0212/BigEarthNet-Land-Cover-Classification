#!/usr/bin/env bash
# Week 2 item 8 of the strengthening plan: run BR-RandomForest across S1, S3
# (5 folds), and S4 (10 countries) -- RF previously only existed for S2, which
# is worthless for the "gap isn't a weak-linear-model artifact" argument.
# 16 Spark jobs total.
set -euo pipefail
...
run_job "s1" "" "e3_full_rf_s1"

for fold in 0 1 2 3 4; do
  run_job "s3" "$fold" "e3_full_rf_s3_fold${fold}"
done
...
echo "=== RF LADDER COMPLETE (16/16 jobs) ==="
